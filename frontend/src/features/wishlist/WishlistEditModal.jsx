/**
 * WishlistEditModal Component
 * @description Modal form to edit existing wishlist item
 * @author KaDong Team
 * @created 2025-11-12
 */
...
const WishlistEditModal = ({ isOpen, onClose, onSuccess, item }) => {
  ...
  useEffect(() => {
    if (item && isOpen) {
      ...
      setImagePreview(item.product_image_url || null)
    }
  }, [item, isOpen])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('[WishlistEditModal] Error updating item:', err)
      alert('Không thể cập nhật sản phẩm: ' + err.message)
    } finally {
      setSubmitting(false)
    }
  }
